...
<header id="site-nav" class="fixed top-0 inset-x-0 z-40 glass border-b border-white/10 transition-colors">
  <nav class="max-w-7xl mx-auto px-6 lg:px-10 h-20 flex items-center justify-between">
    ...
    <div class="hidden lg:flex items-center gap-10 font-medium text-sm">
      <a href="/portfolio.php" class="hover:text-gold transition-colors <?= $activeNav === 'portfolio' ? 'text-gold' : '' ?>">Portfolio</a>
      <a href="/investor-hub.php" class="hover:text-gold transition-colors <?= $activeNav === 'investor-hub' ? 'text-gold' : '' ?>">Investor Hub</a>
      <a href="/reviews.php" class="hover:text-gold transition-colors <?= $activeNav === 'reviews' ? 'text-gold' : '' ?>">Reviews</a>
      <?php if ($navUser): ?>
        <a href="<?= $navUser['role'] === 'admin' ? '/admin/index.php' : '/dashboard.php' ?>" class="hover:text-gold transition-colors <?= $activeNav === 'dashboard' ? 'text-gold' : '' ?>">
    ...
    <div class="hidden lg:flex items-center gap-4">
      <?php if ($navUser): ?>
        <span class="text-sm text-slate">Hi, <?= h(explode(' ', $navUser['name'])[0]) ?></span>
        <a href="/logout.php" class="text-sm font-semibold border border-white/15 hover:border-gold hover:text-gold rounded-full px-5 py-2 transition-colors">Sign Out</a>
      <?php else: ?>
        <a href="/login.php" class="text-sm font-semibold bg-gold text-obsidian hover:bg-gold-light rounded-full px-5 py-2 transition-colors">Investor Login</a>
      <?php endif; ?>
    </div>
  ...
  <div id="mobile-menu" class="hidden lg:hidden border-t border-white/10 px-6 py-6 flex flex-col gap-5 font-medium">
    ...
    <a href="/investor-hub.php" class="hover:text-gold">Investor Hub</a>
    <a href="/reviews.php" class="hover:text-gold">Reviews</a>
    <?php if ($navUser): ?>
      <a href="<?= $navUser['role'] === 'admin' ? '/admin/index.php' : '/dashboard.php' ?>" class="hover:text-gold"><?= $navUser['role'] === 'admin' ? 'Admin Center' : 'My Portal' ?></a>
      <a href="/logout.php" class="hover:text-gold">Sign Out</a>
    <?php else: ?>
      <a href="/login.php" class="inline-block bg-gold text-obsidian text-center rounded-full px-5 py-2.5">Investor Login</a>
    <?php endif; ?>
  </div>
</header>
